...
class HashMap():
    ...
    def put(self, key, value):
        ...
        if self.bucket_array[index_for_value] is not None:
            if isinstance(self.bucket_array[index_for_value],list):
                self.bucket_array[index_for_value].append([key, value])
            else:
                self.bucket_array[index_for_value] = [[key, value]]
        else:
            self.bucket_array[index_for_value] = [[key, value]]
        self.num_entries += 1
    
    def get(self, key, value):
        return self.bucket_array[self.get_bucket_index(key)]
